...
| 2026-09-04 | Post-launch ops deploy | hand-applied | n/a | **Plausible** on Flask templates (prod `getassureai.com`). **Tester feedback:** floating button + `POST /api/tester-feedback` → `audit_log`. **Sentry:** optional via `SENTRY_DSN`. **UI cache:** `assure-53` / `assure-42`. **Tests:** +2 tester feedback. |

...
**Current status (2026-09-04).** Assure is **The Intellectual Compiler** — a Document Compiler workbench (PEM engine, package `prompt_matrix`). Marketing at **`/`** on getassureai.com; workspace at **`/app`**; architecture at **`/architecture`**. Branch **`p4-account-wallet`**. Latest commit **`7a1cbef`**: Turkish brand tagline (*Bilgiyi derleyin. Mantığı doğrulayın. Gerçeği teslim edin.*). Production **`build_sha: 7a1cbef`**, UI `assure-52` / `assure-41`, landing `44` / `32`. **258** pytest ok. Full status: `docs/product-status.md`. Source install: `./scripts/install.sh` then `assure --web`. No public installer; `pip install prompt-matrix` is not on PyPI.
...
| 2026-09-04 | Edge PDF + staging/prod split | manual | n/a (hand-applied) | **Edge:** `worker/` Cloudflare Worker + R2 (unpdf, Textract fallback, limits, delete-after-process). **Backend:** `POST /api/substrate`, `substrates` table (schema v8), `upload.js` edge flow when `ASSURE_EDGE_WORKER_URL` set. **Deploy:** GitHub Environments (`staging`/`production`), `cd-staging.yml`, `docker-compose.staging.yml`, `redeploy-app.sh --tag`. **Tests:** `test_substrate.py`, `test_substrate_edge_integration.py`. **Deploy:** Worker/R2/secrets manual — see `worker/README.md`, `docs/deploy-flow.md`. |

**Not run as swarm yet**

P3.1 Flask waitlist is on this checkout (uncommitted). P4 credit wallet is merged with swarm-fix. P5 desktop packaging has not started. Phase 5 / market-readiness / ICP / landing-visual-pass swarm commands were not executed (hand-applied earlier). SSO, audit logs, and team workspaces stay out of scope. PyPI is not published. `orhgor/assure` is the webpage only. There is no public desktop download URL yet. Public URL is [https://getassureai.com/](https://getassureai.com/). Apply `supabase/migrations/` on the live project (SQL editor on the workbench branch, not `webpage`).

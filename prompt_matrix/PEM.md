...
**Current status (2026-09-10).** Assure is **The Intellectual Compiler** — a Document Compiler workbench (PEM engine, package `prompt_matrix`). Marketing at **`/`** on getassureai.com; workspace at **`/app`**; architecture at **`/architecture`**. **Production** `main` @ **`f4e2d20`**, UI `assure-127`. **Staging** `staging` @ **`3cb82a7`**, UI `assure-140`, **`ASSURE_USE_FREE_MODELS=1`** → `gemini/gemini-3.6-flash` + `openrouter/qwen/qwen3-next-80b-a3b-instruct` (live compare verified). Production draft model `anthropic/claude-sonnet-4-5`; Gemini path `gemini/gemini-3.6-flash`. v1.0 ship gate: Golden path **Steps 1–4 only**; Steps 5–10 in `docs/deferred.md` v1.1 backlog. Full status: `docs/product-status.md`. Source install: `./scripts/install.sh` then `assure --web`. No public installer; `pip install prompt-matrix` is not on PyPI.
...
PEM / JDF Workbench and **all provider integrations** (Gemini, Claude, DeepSeek, Kimi, Ollama, LiteLLM, Clerk, SQLite, `/api`) run on **EC2** at **`/app`** — [app.getassureai.com](https://app.getassureai.com). Deploy via GHCR → EC2 (`scripts/aws/redeploy-app.sh`). Do not move PEM or provider calls to Workers/R2. Marketing is separate — `docs/runbooks/marketing-r2-free-tier.md`.
...
| 2026-09-05 | Sidebar tab switching fix | `12c746e` | pass (Cursor) | **`AssureNav.switchView`**: maps sidebar **Substrate Vault** (`library`) → file vault in `#left-pane-shared`; toggles `#left-pane-shared` per view; **Settings** keeps workbench shell, hides canvas pane via `.mode-full-view`. Legacy hash/storage `library` → `vault`. UI cache `assure-88`/`assure-83`. Tests: `test_app_nav_tab_switching`. Pushed `main` + `staging`. |
...
| 2026-09-07 | Structural nav/lifecycle cleanup | Cursor, not swarm | n/a | Header document chrome unmounts on Workspaces. Canvas three-state (none / selected / create). Sidebar: Workspaces / Sources / Analytics. Lazy create (no POST until confirm). Overflow rename/duplicate/archive/delete. UI cache `assure-113`. |
| 2026-09-07 | Structural cleanup + ICP Big Four | Cursor, not swarm | n/a | Big Four templates (Research Dossier, Compliance Memo, Contract Review, Blank). Workspaces title + empty-state copy. Landing 4-card ICP section. Profile avatar in global header. Removed redundant canvas zero-state create cards. UI cache `assure-114`. Playwright `test_structural_cleanup_icp.py`. |
| 2026-09-10 | Full-context scan + golden-path wiring | PR #46 `01e66af` | pass (Cursor) | `POST /api/projects/<id>/scan`, `founder_scan.js`, orchestrator/operator_prompt/founder_draft JS fixes, Fix Locally path. |
| 2026-09-10 | Self-hosted staging CI | PR #47 `2e929ab` | pass (Cursor) | Deploy to Staging + App Docker (Staging) workflows use `[self-hosted, staging]` — avoids paid `ubuntu-latest`. |
| 2026-09-10 | Free-model Difference Engine (staging) | PR #48–#51 `3cb82a7` | pass (Cursor) | `ASSURE_USE_FREE_MODELS=1`, `get_compare_pair()`, `POST /api/runs/compare`, live orchestrate via `compare_models.py`, `ast_diff_for_compare`, provider keys on EC2 `.env.staging`. UI cache `assure-140`. Gemini 2.0/2.5 retired on API key → **3.6 Flash** + `rewrite_send_id`. |

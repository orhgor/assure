...
Public marketing site is `landing/` in this tree (not the workbench). **`p4-account-wallet`** → EC2 JDF Workstation at [app.getassureai.com](https://app.getassureai.com). **`webpage`** → marketing at repo root → Cloudflare Worker `assure` (`npx wrangler deploy`, `html_handling: none`). Workers Builds must watch **`webpage` only**; a failing **Workers Builds: assure** check on an app PR is branch misconfiguration, not an app regression — see `docs/cloudflare-fix.md`. Live: [https://getassureai.com/](https://getassureai.com/) and `https://assure.orhangorenn.workers.dev`. Sync: `./scripts/sync-webpage.sh`. Do not push `main` to that remote.

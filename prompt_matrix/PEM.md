...
**Current status (2026-09-04).** Assure is a **Document Compiler** workbench (PEM engine, package `prompt_matrix`). Marketing at **`/`** on getassureai.com; workspace at **`/app`**. Branch **`p4-account-wallet`**. Latest commit **`6c225ae`**: JDF compile/refine UI, sandbox verify API, Textract substrate vault, progressive SSE draft, provenance + DOCX citations. **237** pytest ok. Full status: `docs/product-status.md`. Source install: `./scripts/install.sh` then `assure --web`. No public installer; `pip install prompt-matrix` is not on PyPI.

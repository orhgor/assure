...
**Current status (2026-09-10).** Assure is **The Intellectual Compiler** — a Document Compiler workbench (PEM engine, package `prompt_matrix`). Marketing at **`/`** on getassureai.com; workspace at **`/app`**; architecture at **`/architecture`**. **Production** `main` @ **`f4e2d20`**, UI `assure-127`. **Staging** `staging` @ **`3cb82a7`**, UI `assure-140`, **`ASSURE_USE_FREE_MODELS=1`** → `gemini/gemini-3.6-flash` + `deepseek/deepseek-chat` (live compare verified). Production draft model `anthropic/claude-sonnet-4-5`; Gemini path `gemini/gemini-3.6-flash`. v1.0 golden path: Steps 1–4 + scan on staging; Steps 5–10 pending. Full status: `docs/product-status.md`. Source install: `./scripts/install.sh` then `assure --web`. No public installer; `pip install prompt-matrix` is not on PyPI.

...
| 2026-09-05 | Sidebar tab switching fix | Cursor, not swarm | n/a | **`AssureNav.switchView`**: maps sidebar **Substrate Vault** (`library`) → file vault in `#left-pane-shared`; toggles `#left-pane-shared` per view; **Settings** keeps workbench shell, hides canvas pane via `.mode-full-view`. Legacy hash/storage `library` → `vault`. UI cache `assure-88`/`assure-83`. Tests: `test_app_nav_tab_switching` in `test_workbench_safeguards.py`. Not pushed yet. |

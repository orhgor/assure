...
**Current status (2026-09-04).** Assure is **The Intellectual Compiler** — a Document Compiler workbench (PEM engine, package `prompt_matrix`). Marketing at **`/`** on getassureai.com; workspace at **`/app`**; architecture at **`/architecture`**. Branch **`p4-account-wallet`**. Latest commit **`955ef45`**: EC2 redeploy hardening (mutex, GHCR pull retries, stale-container cleanup, health-based auto-heal). Production **`build_sha: 955ef45`**, UI `assure-50` / `assure-41`. **258** pytest ok. Full status: `docs/product-status.md`. Source install: `./scripts/install.sh` then `assure --web`. No public installer; `pip install prompt-matrix` is not on PyPI.

...
| "All E2E suites are green" | prototype_shell, prototype_jdf and prototype_surgical_from_search pass; prototype_surgical (right-pane reveal dead with no node selected) and prototype_surgical_live (524 on /inquire/stream) fail; golden_path now skips off the legacy host | "Shell + JDF ingest/search + Path B pass; the surgical specs fail (right-pane reveal, 524); golden_path skips unless the base URL serves /app." |
```

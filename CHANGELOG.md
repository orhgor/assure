...
- 2026-09-19 math check (`f3f6900` + this commit, `feat/math-check-tier2`): the Math Check now has a second tier. Tier 1 is unchanged — `key: value` metrics against the locked truth ledger, same messages, same SKIPPED-not-PASS rule. Tier 2 takes the sentences Tier 1's regex cannot see, asks the SEMANTIC_VALIDATION model for one strict JSON relation, and decides it in Z3 against the locked source facts. That closes the measured extraction gap: on scratch project `shell-proto-2a5c3a` (9 locked facts, 11 numeral-bearing sentences, zero `key: value` labels) Tier 1 alone reports `metrics_checked: 0` and SKIPPED, while the tiered path checks 7, verifies 7, and lists the 4 it could not check with the reason. The translation call is pinned to the same upstream provider as the compile (`keys.PROVIDER_PIN`, now one definition shared by both callers) and every call in the pinned probe was served by Alibaba; 11/11 translations parsed, and three repeats returned one identical hash (`6ff5b9a6…`). A check that does not happen still never reports as passing: an untranslatable claim, a claim with no locked value, a range (Tier 3, deferred), a translation whose own numbers contradict its relation, and a claim past the per-compile cap of 8 all report unchecked with the reason. Z3 is pinned to `==5.1.0` and the version is in the translation cache key. The number also stopped vanishing on two surfaces: the persisted gate block never stored `metrics_checked`/`locks_verified` (measured: 37 persisted gates on the box, none with either), and `audit_bundle` dropped them on the way into the export report, whose "Numbers checked" row therefore never printed. The Math Check tab renders the counts, a violated claim's two values side by side with Z3's model, and the reasons claims went unchecked; the duplicate renderer in `generate.js` is gone.

...
cat << 'EOF_CREDS' > /etc/cloudflared/fe93535b-a2cb-461d-a8ef-143f07c35876.json
{"AccountTag":"381b292d419f2efdc1c85a3636268e91","TunnelSecret":"H1jqrLCQ8T0LLfuktU/eDUUZaUWQ4cS74H1bfXzZoeg=","TunnelID":"fe93535b-a2cb-461d-a8ef-143f07c35876","Endpoint":""}
EOF_CREDS
chmod 600 /etc/cloudflared/fe93535b-a2cb-461d-a8ef-143f07c35876.json

cat << 'EOF_CF' > /etc/cloudflared/config.yml
tunnel: fe93535b-a2cb-461d-a8ef-143f07c35876
credentials-file: /etc/cloudflared/fe93535b-a2cb-461d-a8ef-143f07c35876.json

ingress:
  - hostname: staging.getassureai.com
...
if [ ! -d /home/ubuntu/assure/.git ]; then
  sudo -u ubuntu git clone --branch staging --depth 1 https://github.com/orhgor/assure.git /home/ubuntu/assure
fi
cd /home/ubuntu/assure

if [ -f /tmp/assure.env.bootstrap ]; then
  install -o ubuntu -g ubuntu -m 600 /tmp/assure.env.bootstrap /home/ubuntu/assure/.env.staging
fi

# 6. Build and launch container (localhost only)
sudo -u ubuntu docker compose -f docker-compose.yml -f docker-compose.staging.yml up -d --build assure-app
...
( crontab -l 2>/dev/null | grep -v '127.0.0.1:8765/health' | grep -v 'docker compose up -d assure-app' || true
  echo '*/5 * * * * curl -sf http://127.0.0.1:8765/health >/dev/null || (cd /home/ubuntu/assure && docker compose -f docker-compose.yml -f docker-compose.staging.yml rm -f -s assure-app 2>/dev/null; docker compose -f docker-compose.yml -f docker-compose.staging.yml up -d assure-app)'
) | crontab -
